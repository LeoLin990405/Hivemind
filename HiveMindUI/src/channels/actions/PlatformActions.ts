...
export const handlePairingShow: ActionHandler = async (context) => {
  ...
  try {
    ...
    return createSuccessResponse({
      type: 'text',
      text: ['🔗 <b>Device Pairing</b>', '', 'Please approve this pairing request in the HiveMind app:', '', `<code>${code}</code>`, '', `⏱ Valid for: ${expiresInMinutes} minutes`, '', '<b>Steps:</b>', '1. Open HiveMind app', '2. Go to WebUI → Channels', '3. Click "Approve" in pending pairing requests'].join('\n'),
      parseMode: 'HTML',
...
export const handlePairingRefresh: ActionHandler = async (context) => {
  ...
  try {
    ...
    return createSuccessResponse({
      type: 'text',
      text: ['🔄 <b>New Pairing Code</b>', '', `<code>${code}</code>`, '', `⏱ Valid for: ${expiresInMinutes} minutes`, '', 'Please approve this pairing request in HiveMind settings.'].join('\n'),
      parseMode: 'HTML',
      replyMarkup: getPairingCodeMarkup(platform, code),
    });
  } catch (error: any) {
...
export const handlePairingCheck: ActionHandler = async (context) => {
  ...
  if (pendingRequest) {
    ...
    return createSuccessResponse({
      type: 'text',
      text: ['⏳ <b>Waiting for Approval</b>', '', `Pairing code: <code>${pendingRequest.code}</code>`, `Time remaining: ${expiresInMinutes} minutes`, '', 'Please approve the pairing request in HiveMind settings.'].join('\n'),
      parseMode: 'HTML',
...
export const handlePairingHelp: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: ['❓ <b>Pairing Help</b>', '', '<b>What is pairing?</b>', `Pairing links your ${platformName} account with the local HiveMind application.`, 'You need to pair before using the AI assistant.', '', '<b>Pairing steps:</b>', '1. Get pairing code (send any message)', '2. Open HiveMind app', '3. Go to WebUI → Channels', '4. Click "Approve" in pending requests', '', '<b>FAQ:</b>', '• Pairing code valid for 10 minutes, refresh if expired', '• HiveMind app must be running', '• Ensure network connection is stable'].join('\n'),
    parseMode: 'HTML',
