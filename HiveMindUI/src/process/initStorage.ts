...
const shouldLogMissingBuiltinResources = process.env.HIVEMIND_DEBUG_RESOURCES === '1';
...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  const missingBuiltinDirs = new Set<string>();
  ...
  const resolveBuiltinDir = (dirPath: string): string => {
    ...
    if (shouldLogMissingBuiltinResources && !missingBuiltinDirs.has(dirPath)) {
      missingBuiltinDirs.add(dirPath);
      ...
      for (const [locale, ruleFile] of Object.entries(preset.ruleFiles)) {
        try {
          ...
          if (!existsSync(sourceRulesPath)) {
            if (shouldLogMissingBuiltinResources) {
