 ...
 * - HiveMindMcpAgent: 管理 HiveMind 本地 @office-ai/aioncli-core 的运行时 MCP 配置
 */
export class HiveMindMcpAgent extends AbstractMcpAgent {
  constructor() {
    // 使用 'hivemind' 作为 backend type 来区分真实的 Gemini CLI
  ...
  async detectMcpServers(_cliPath?: string): Promise<IMcpServer[]> {
    try {
      ...
      console.warn('[HiveMindMcpAgent] Failed to detect MCP servers:', error);
      return [];
    }
  }

  /**
   * 安装 MCP 服务器到 HiveMind 配置
    ...
    try {
      ...
      mcpServers.forEach((server) => {
        ...
        if (this.getSupportedTransports().includes(server.transport.type)) {
          ...
          console.warn(`[HiveMindMcpAgent] Skipping ${server.name}: unsupported transport type ${server.transport.type}`);
        }
      });
      ...
      console.log('[HiveMindMcpAgent] Installed MCP servers:', mcpServers.map((s) => s.name).join(', '));
      return { success: true };
    } catch (error) {
      console.error('[HiveMindMcpAgent] Failed to install MCP servers:', error);
      return { success: false, error: error instanceof Error ? error.message : String(error) };
    }
  }

  /**
   * 从 HiveMind 配置中移除 MCP 服务器
   *
   * 注意：HiveMind 的 MCP 配置由前端（renderer 层）统一管理
   * 这里不做任何操作，因为：
   * 1. Toggle 关闭时：前端已经设置 enabled: false，不需要后端再次修改
   * 2. 删除服务器时：前端已经从配置中删除，不需要后端再次删除
   *
   * HiveMindMcpAgent 只负责读取配置（detectMcpServers）和添加配置（installMcpServers），
   * 不应该在 remove 流程中修改配置，避免与前端的配置管理产生冲突
   */
  removeMcpServer(mcpServerName: string): Promise<McpOperationResult> {
    console.log(`[HiveMindMcpAgent] Skip removing '${mcpServerName}' - config managed by renderer`);
