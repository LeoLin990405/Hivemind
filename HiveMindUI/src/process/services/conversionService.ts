...
// Dynamic Electron import for BrowserWindow (used for HTML→PDF conversion)
let BrowserWindow: typeof import('electron').BrowserWindow | null = null;
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  BrowserWindow = require('electron').BrowserWindow;
  if (typeof BrowserWindow?.prototype?.loadURL !== 'function') BrowserWindow = null;
} catch {
  BrowserWindow = null;
}
...
class ConversionService {
  ...
  public async htmlToPdf(html: string, targetPath: string): Promise<ConversionResult<void>> {
    if (!BrowserWindow) {
      return { success: false, error: 'HTML to PDF conversion requires Electron (BrowserWindow not available in standalone mode)' };
    }
