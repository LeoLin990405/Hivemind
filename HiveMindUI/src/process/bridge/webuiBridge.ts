...
export function initWebuiBridge(): void {
  ...
  // Only register when Electron ipcMain is available
  if (!_ipcMain) return;

  // 直接 IPC: 重置密码 / Direct IPC: Reset password
  _ipcMain.handle('webui-direct-reset-password', async () => {
    return WebuiService.handleAsync(async () => {
      const newPassword = await WebuiService.resetPassword();
  ...
  _ipcMain.handle('webui-direct-get-status', async () => {
    return WebuiService.handleAsync(async () => {
      const status = await WebuiService.getStatus(webServerInstance);
      return { success: true, data: status };
  ...
  _ipcMain.handle('webui-direct-change-password', async (_event, { newPassword }: { newPassword: string }) => {
    return WebuiService.handleAsync(async () => {
      await WebuiService.changePassword(newPassword);
      return { success: true };
  ...
  _ipcMain.handle('webui-direct-generate-qr-token', async () => {
    // 检查 webServerInstance 状态
    if (!webServerInstance) {
      return {
        success: false,
