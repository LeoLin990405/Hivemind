...
// Dynamic Electron import — returns null in standalone mode
let _electronApp: typeof import('electron').app | null = null;
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  _electronApp = require('electron').app;
  // Verify it's a real Electron app (not a stub)
  if (typeof _electronApp?.getPath !== 'function') _electronApp = null;
} catch {
  _electronApp = null;
}
...
export const getTempPath = () => {
  if (_electronApp) {
    const rootPath = _electronApp.getPath('temp');
    return path.join(rootPath, 'hivemind');
  }
  return paths.getAppTempPath();
};
...
const ensureCliSafeSymlink = (targetPath: string, symlinkName: string): string => {
  ...
  const homePath = _electronApp ? _electronApp.getPath('home') : paths.getAppHomePath();
