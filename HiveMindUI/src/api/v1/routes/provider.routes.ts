...
import { z } from 'zod';
import { providerSchema, createProviderRequestSchema, updateProviderRequestSchema } from '../schemas/model';
 ...
 * Create new provider
 */
router.post('/', validateRequest({ body: createProviderRequestSchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.post('/:id/test', validateRequest({ params: z.object({ id: z.string().uuid() }) }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
