...
/** HiveModal 组件 Props */
export interface HiveModalProps extends Omit<ModalProps, 'title' | 'footer'> {
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <HiveModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </HiveModal>
 *
 * // 预设尺寸 / Preset size
 * <HiveModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </HiveModal>
 *
...
const HiveModal: React.FC<HiveModalProps> = ({
  children,
  size,
  header,
  footer,
  ...
  return (
    <Modal {...props} title={null} closable={false} footer={null} onCancel={onCancel} className={`hivemind-modal ${className}`} style={finalStyle} getPopupContainer={() => document.body}>
      <div className='hivemind-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='hivemind-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
