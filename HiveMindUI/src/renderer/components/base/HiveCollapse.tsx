 ...
 */
export interface HiveCollapseProps {
 ...
 */
export interface HiveCollapseItemProps {
 ...
 */
const HiveCollapseItem: React.FC<HiveCollapseItemProps> = ({ children }) => <>{children}</>;
HiveCollapseItem.displayName = 'HiveCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <HiveCollapse defaultActiveKey={['1']}>
 *   <HiveCollapse.Item name="1" header="面板1">
 *     内容1
 *   </HiveCollapse.Item>
 *   <HiveCollapse.Item name="2" header="面板2">
 *     内容2
 *   </HiveCollapse.Item>
 * </HiveCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <HiveCollapse accordion defaultActiveKey="1">
 *   <HiveCollapse.Item name="1" header="面板1">内容1</HiveCollapse.Item>
 *   <HiveCollapse.Item name="2" header="面板2">内容2</HiveCollapse.Item>
 * </HiveCollapse>
 *
 * // 自定义图标 / Custom icon
 * <HiveCollapse
 ...
 * ```
 */
const HiveCollapseComponent: React.FC<HiveCollapseProps> & { Item: typeof HiveCollapseItem } = ({ children, className, defaultActiveKey, activeKey, onChange, accordion, expandIcon, expandIconPosition = 'left', bordered = true }) => {
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<HiveCollapseItemProps> => {
      return React.isValidElement(child) && child.type === HiveCollapseItem;
    });
  }, [children]);
...
HiveCollapseComponent.Item = HiveCollapseItem;

export default HiveCollapseComponent;
