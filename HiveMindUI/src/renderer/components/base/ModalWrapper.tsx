...
const ModalWrapper: React.FC<ModalWrapperProps> = ({ children, title, showCustomClose = true, onCancel, className = '', ...props }) => {
  return (
    <Modal {...props} title={null} closable={false} onCancel={onCancel} className={`hivemind-modal ${className}`}>
      <div>
        {showCustomClose && title && (
          <div className='hivemind-modal-header'>
            <h3 className='hivemind-modal-title'>{title}</h3>
            <button onClick={onCancel} className='hivemind-modal-close-btn'>
