...
const WebuiModalContent: React.FC = () => {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      // 桌面模式直接走 bridge，避免 /api/v1/webui/status 在该模式下不可用
      // Desktop mode: use bridge directly to avoid unavailable /api/v1/webui/status endpoint
  ...
  const handleSetNewPassword = async () => {
    try {
      ...
      if (isDesktop) {
        ...
        // 浏览器模式优先 HTTP API，失败时回退 bridge
        // Browser mode: prefer HTTP API, fallback to bridge
        try {
