...
const setDraft = <K extends TChatConversation['type']>(type: K, conversation_id: string, draft: Extract<Draft, { _type: K }> | undefined) => {
  ...
  switch (type) {
    ...
    case 'hivemind':
      if (draft) {
        store.hivemind.set(conversation_id, draft as Extract<Draft, { _type: 'hivemind' }>);
      } else {
        store.hivemind.delete(conversation_id);
      }
      break;
    default:
      break;
...
const getDraft = <K extends TChatConversation['type']>(type: K, conversation_id: string): Extract<Draft, { _type: K }> | undefined => {
  ...
  switch (type) {
    ...
    case 'hivemind':
      return store.hivemind.get(conversation_id) as Extract<Draft, { _type: K }>;
    default:
      return undefined;
  }
};
