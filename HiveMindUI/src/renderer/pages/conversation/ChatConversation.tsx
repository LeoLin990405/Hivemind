...
import AcpModelSelector from './acp/AcpModelSelector';
import ChatLayout from './ChatLayout';
import ChatSider from './ChatSider';
import CodexChat from './codex/CodexChat';
import CodexModelSelector from './codex/CodexModelSelector';
import OpenClawChat from './openclaw/OpenClawChat';
import OpenClawModelSelector from './openclaw/OpenClawModelSelector';
...
import HivemindModelSelector from './hivemind/HivemindModelSelector';
...
const ChatConversation: React.FC<{
  ...
  const navigate = useNavigate();
  const workspaceEnabled = Boolean(conversation?.extra?.workspace);
  const isMergedHivemindConversation = conversation?.type === 'hivemind' && (conversation.extra as { provider?: string } | undefined)?.provider !== 'obsidian';

  const isGeminiConversation = conversation?.type === 'gemini';

  React.useEffect(() => {
    if (!isMergedHivemindConversation) return;
    void Promise.resolve(navigate('/agent-teams/chat', { replace: true })).catch((error) => {
      console.error('Navigation failed:', error);
    });
  }, [isMergedHivemindConversation, navigate]);

  if (isMergedHivemindConversation) {
    return null;
  }
  ...
  // 为 ACP 会话添加模型选择器
  // Add model selector for ACP conversations
  const currentAcpModelId = (conversation?.extra as { modelId?: string } | undefined)?.modelId;
  const acpModelSelector = conversation?.type === 'acp' && conversation?.extra?.backend ? <AcpModelSelector conversationId={conversation.id} backend={conversation.extra.backend} currentModelId={currentAcpModelId} /> : undefined;

  // 为 Codex 会话添加模型选择器
  const codexModelSelector = conversation?.type === 'codex' ? <CodexModelSelector conversationId={conversation.id} /> : undefined;

  // 为 OpenClaw 会话添加模型选择器
  const openclawModelSelector = conversation?.type === 'openclaw-gateway' ? <OpenClawModelSelector conversationId={conversation.id} /> : undefined;
  const hivemindModelSelector = conversation?.type === 'hivemind' ? <HivemindModelSelector conversationId={conversation.id} /> : undefined;
  const headerLeftNode = acpModelSelector || codexModelSelector || openclawModelSelector || hivemindModelSelector;
  const headerExtraNode = conversation ? <CronJobManager conversationId={conversation.id} /> : undefined;
  const siderNode = <ChatSider conversation={conversation} />;

  return (
    <ChatLayout title={conversation?.name} {...chatLayoutProps} headerLeft={headerLeftNode} headerExtra={headerExtraNode} siderTitle={sliderTitle} sider={siderNode} workspaceEnabled={workspaceEnabled}>
      {conversationNode}
    </ChatLayout>
  );
};
