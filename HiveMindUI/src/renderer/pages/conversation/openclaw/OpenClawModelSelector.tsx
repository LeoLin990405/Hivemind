...
const OpenClawModelSelector: React.FC<{ conversationId: string }> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const fallback = await ipcBridge.models.getDefaultModel.invoke({ provider: OPENCLAW_PROVIDER });
  ...
  useEffect(() => {
    ...
    emitter.on('openclaw.model.changed', handler);
    return () => { emitter.off('openclaw.model.changed', handler); };
  }, []);

  const handleChange = useCallback(async (modelId: string) => {
    setPreferredModel(modelId);
    emitter.emit('openclaw.model.changed', modelId);
    try {
      const prefs = await ipcBridge.models.getUserPreferences.invoke();
      await ipcBridge.models.saveUserPreferences.invoke({
        selectedModels: { ...(prefs.selectedModels || {}), [OPENCLAW_PROVIDER]: modelId },
        lastUpdated: prefs.lastUpdated,
      });
    } catch (error) {
      console.error('[OpenClawModelSelector] Failed to save model:', error);
    }
  }, []);
