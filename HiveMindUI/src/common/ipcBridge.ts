...
export const agentTeams = {
  ...
  // Team Chat: real-time conversation within a team via Gateway API
  teamChat: {
    send: bridge.buildProvider<IBridgeResponse<{ msg_id: string }>, { team_id: string; message: string; provider?: string | null; model?: string | null; files?: string[] }>('agent-teams.team-chat.send'),
    responseStream: bridge.buildEmitter<IResponseMessage & { team_id: string }>('agent-teams.team-chat.response-stream'),
  },
};
