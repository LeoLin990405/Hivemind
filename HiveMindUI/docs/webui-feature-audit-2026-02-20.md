...
11. WebUI/Backend 与 App 启动链解耦（standalone）
- 代码：`src/process/initBridge.server.ts`、`src/server.ts`
- 结果：standalone 启动不再因单个桥接依赖失败而整体崩溃；非关键模块失败时自动跳过并继续启动

12. WebUI 启动入口切换为 standalone 工作流
- 代码：`package.json`、`scripts/start-webui-standalone.js`
- 结果：`npm run webui` 默认不再走 Electron Forge，改为“Web 前端 + 独立后端”并行启动

13. 创建账号密码能力（Web）
- 代码：`src/webserver/routes/authRoutes.ts`、`src/renderer/context/AuthContext.tsx`、`src/renderer/pages/login/index.tsx`
- 结果：新增 `POST /api/auth/register` 与登录页“Create account”模式，创建成功后自动登录

### 0.2 当前剩余阻塞
1. 无 P0 阻塞项（当前版本可用）
2. 非阻塞项：`/settings/webui` Slack/Discord 仍为 `Coming Soon`
3. 非阻塞项：`/monitor` 与 `/agent-teams/monitor` 仍是双入口，但文案已区分系统/团队
4. 非阻塞项：部分依赖链（`@office-ai/aioncli-core` 相关）在 standalone 下可能触发桥接自动降级（`auth/gemini/conversation/channel`）

## 1. 测试范围与方法
- 测试目标：`/Users/leo/.local/share/codex-dual/HiveMindUI` WebUI 路由与核心交互
