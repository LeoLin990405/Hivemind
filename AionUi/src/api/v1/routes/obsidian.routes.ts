 ...
 */
router.get('/vaults', validateRequest({ query: paginationQuerySchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.post('/vaults/:id/sync', validateRequest({ params: z.object({ id: z.string().uuid() }) }), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
