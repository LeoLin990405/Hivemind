 ...
 */
router.post('/change-password', authenticateJWT, requireRole('admin'), validateRequest({ body: changePasswordSchema }), async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.post('/reset-password', authenticateJWT, requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.post('/qr-token', authenticateJWT, async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/config', authenticateJWT, requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const config = {
      enabled: process.env.WEBUI_ENABLED === 'true',
      port: parseInt(process.env.WEBUI_PORT || '8080', 10),
 ...
 */
router.put('/config', authenticateJWT, requireRole('admin'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { enabled, port, host, requireAuth, qrEnabled, maxConnections, allowedOrigins } = req.body;
