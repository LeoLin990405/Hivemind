...
export function createMainMenuCard(): LarkCard {
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: 'HiveMind Assistant' },
      template: 'blue',
    },
    elements: [
      {
...
export function createPairingCard(pairingCode: string): LarkCard {
  return {
    ...
    elements: [
      {
        tag: 'markdown',
        content: ['Please pair your account with HiveMind:', '', `**Pairing Code:** \`${pairingCode}\``, '', '1. Open HiveMind settings', '2. Go to Channels → Lark', '3. Enter this pairing code', '', 'Code expires in 10 minutes.'].join('\n'),
      },
      {
        tag: 'action',
        actions: [
...
export function createPairingStatusCard(pairingCode: string): LarkCard {
  return {
    ...
    elements: [
      {
        tag: 'markdown',
        content: ['Your pairing request is pending approval.', '', `**Pairing Code:** \`${pairingCode}\``, '', 'Please approve in HiveMind settings:', '1. Open HiveMind app', '2. Go to WebUI → Channels', '3. Click "Approve" for this code'].join('\n'),
      },
      {
        tag: 'action',
        actions: [
...
export function createPairingHelpCard(): LarkCard {
  return {
    ...
    elements: [
      {
        tag: 'markdown',
        content: ['**What is pairing?**', 'Pairing links your Lark/Feishu account with the local HiveMind application.', 'You need to pair before using the AI assistant.', '', '**How to pair:**', '1. Send any message to this bot', '2. You will receive a pairing code', '3. Open HiveMind desktop app', '4. Go to WebUI → Channels → Lark', '5. Click "Approve" for your code', '', '**FAQ:**', '• Pairing code valid for 10 minutes', '• HiveMind app must be running', '• One account can only pair once'].join('\n'),
      },
      {
        tag: 'action',
        actions: [
...
export function createHelpCard(): LarkCard {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: '❓ HiveMind Assistant Help' },
      template: 'turquoise',
    },
    elements: [
      {
        tag: 'markdown',
        content: ['A remote assistant to interact with HiveMind via Lark.', '', '**Common Actions:**', '• 🆕 New Chat - Start a new session', '• 🔄 Agent - Switch AI agent', '• 📊 Status - View current session status', '• ❓ Help - Show this help message', '', 'Send a message to chat with the AI assistant.'].join('\n'),
      },
      {
        tag: 'action',
        actions: [
...
export function createPairingGuideCard(): LarkCard {
  return {
    ...
    elements: [
      {
        tag: 'markdown',
        content: ['**First-time Setup:**', '1. Send any message to the bot', '2. Bot displays pairing code', '3. Approve pairing in HiveMind settings', '4. Ready to use after pairing', '', '**Notes:**', '• Pairing code valid for 10 minutes', '• HiveMind app must be running', '• One Lark account can only pair once'].join('\n'),
      },
      {
        tag: 'action',
        actions: [
...
export function createSettingsCard(): LarkCard {
  return {
    ...
    elements: [
      {
        tag: 'markdown',
        content: ['Channel settings need to be configured in the HiveMind app.', '', 'Open HiveMind → WebUI → Channels'].join('\n'),
      },
      {
        tag: 'action',
        actions: [
