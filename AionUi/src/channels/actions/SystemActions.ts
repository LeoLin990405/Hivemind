...
export const handleHelpShow: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: ['❓ <b>HiveMind Assistant</b>', '', 'A remote assistant to interact with HiveMind via Telegram.', '', '<b>Common Actions:</b>', '• 🆕 New Chat - Start a new session', '• 📊 Status - View current session status', '• ❓ Help - Show this help message', '', 'Send a message to chat with the AI assistant.'].join('\n'),
    parseMode: 'HTML',
...
export const handleHelpPairing: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: ['🔗 <b>Pairing Guide</b>', '', '<b>First-time Setup:</b>', '1. Send any message to the bot', '2. Bot displays pairing code', '3. Approve pairing in HiveMind settings', '4. Ready to use after pairing', '', '<b>Notes:</b>', '• Pairing code valid for 10 minutes', '• HiveMind app must be running', '• One Telegram account can only pair once'].join('\n'),
    parseMode: 'HTML',
    replyMarkup: createHelpKeyboard(),
  });
};
...
export const handleSettingsShow: ActionHandler = async (context) => {
  ...
  return createSuccessResponse({
    type: 'text',
    text: ['⚙️ <b>Settings</b>', '', 'Channel settings need to be configured in the HiveMind app.', '', 'Open HiveMind → WebUI → Channels'].join('\n'),
    parseMode: 'HTML',
