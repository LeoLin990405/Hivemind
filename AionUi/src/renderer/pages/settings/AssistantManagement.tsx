...
// Phoenix Migration: Partially migrated (85%)
// Complex modals (Skills, Delete Pending/Custom Skill) still use Arco Modal
import type { Message } from '@arco-design/web-react';
import { Avatar, Collapse, Drawer, Modal } from '@arco-design/web-react';
    ...
    <div>
        ...
        footer={
          <div className='flex items-center justify-between w-full'>
            <div className='flex items-center gap-8px'>
              <Button variant='default' onClick={handleSave} className='w-[100px] rounded-[100px]'>
            ...
            {!isCreating && !activeAssistant?.isBuiltin && (
              <Button variant='destructive' onClick={handleDeleteClick} className='rounded-[100px]' style={{ backgroundColor: 'rgb(var(--danger-1))' }}>
          ...
          <div className='flex flex-col flex-1 gap-16px bg-fill-2 rounded-16px p-20px overflow-y-auto'>
            <div className='flex-shrink-0'>
              ...
              <div className='mt-10px flex items-center gap-12px'>
                ...
                <Input value={editName} onChange={(e) => setEditName(e.target.value)} disabled={activeAssistant?.isBuiltin} placeholder={t('settings.agentNamePlaceholder', { defaultValue: 'Enter a name for this agent' })} className='flex-1 rounded-4px bg-bg-1' />
              </div>
            </div>
            <div className='flex-shrink-0'>
              <span className='font-medium'>{t('settings.assistantDescription', { defaultValue: 'Assistant Description' })}</span>
              <Input className='mt-10px rounded-4px bg-bg-1' value={editDescription} onChange={(e) => setEditDescription(e.target.value)} disabled={activeAssistant?.isBuiltin} placeholder={t('settings.assistantDescriptionPlaceholder', { defaultValue: 'What can this assistant help with?' })} />
            </div>
            <div className='flex-shrink-0'>
            ...
            {(isCreating || (activeAssistantId && hasBuiltinSkills(activeAssistantId)) || (activeAssistant && !activeAssistant.isBuiltin)) && (
              <div className='flex-shrink-0 mt-16px'>
                <div className='flex items-center justify-between mb-12px'>
                  <span className='font-medium'>{t('settings.assistantSkills', { defaultValue: 'Skills' })}</span>
                  <Button size='sm' variant='outline' onClick={() => setSkillsModalVisible(true)} className='rounded-[100px] gap-1'>
                    <Plus size={14} />
      ...
      >
        <div className='space-y-16px'>
          ...
          <div className='space-y-12px'>
            <span>{t('settings.skillFolderPath', { defaultValue: 'Skill Folder Path' })}</span>
            <div className='flex items-center gap-8px'>
              <Input value={skillPath} onChange={(e) => setSkillPath(e.target.value)} placeholder={t('settings.skillPathPlaceholder', { defaultValue: 'Enter or browse skill folder path' })} className='flex-1' />
              <Button
                variant='outline'
                className='gap-1'
                onClick={async () => {
