...
import { IconExclamationCircle, IconCloseCircle, IconRefresh, IconWifi } from '@arco-design/web-react/icon';
...
export function PermissionError({ message = 'You do not have permission to access this resource.', onBack }: PermissionErrorProps) {
  return (
    <Result
      status='403'
      title='Access Denied'
...
export function QueryError({ error, onRetry }: QueryErrorProps) {
  const isNetworkError = error instanceof Error && (error.message.includes('network') || error.message.includes('fetch') || error.message.includes('Network'));

  if (isNetworkError) {
    return <NetworkError onRetry={onRetry} />;
  }

  const is404 = error instanceof Error && ('status' in error ? (error as any).status === 404 : error.message.includes('not found'));

  if (is404) {
    return <NotFoundError onBack={onRetry} />;
  }

  const is403 = error instanceof Error && ('status' in error ? (error as any).status === 403 : error.message.includes('permission'));
...
export function LoadingError({ error, resetErrorBoundary }: LoadingErrorProps) {
  return (
    <div className='loading-error flex flex-col items-center justify-center min-h-[200px] p-8'>
      <IconExclamationCircle style={{ fontSize: 48, color: 'var(--color-danger-6)', marginBottom: 16 }} />
      <div className='text-lg font-semibold mb-2'>Failed to load</div>
      <div className='text-gray-500 mb-4'>{error instanceof Error ? error.message : 'An unexpected error occurred'}</div>
      {resetErrorBoundary && (
