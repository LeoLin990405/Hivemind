    ...
    <div className='flex flex-col gap-16px min-h-0'>
      ...
      <div className='flex-1 min-h-0'>
        {mcpServers.length === 0 ? (
          ...
          <HiveScrollArea className={classNames('max-h-360px', isPageMode && 'max-h-none')} disableOverflow={isPageMode}>
      ...
      <HiveScrollArea className='flex-1 min-h-0 pb-16px' disableOverflow={isPageMode}>
        <div className='space-y-16px'>
          ...
          <div className='px-[12px] md:px-[32px] py-[24px] bg-2 rd-12px md:rd-16px flex flex-col min-h-0 border border-border-2'>
            <div className='flex-1 min-h-0'>
              <HiveScrollArea className={classNames('h-full', isPageMode && 'overflow-visible')} disableOverflow={isPageMode}>
                ...
                {imageGenerationModelList.length > 0 ? (
                  ...
                  >
                    {imageGenerationModelList.map(({ model, ...platform }) => (
                      <HiveSelect.OptGroup label={platform.name} key={platform.id}>
                        {model.map((modelName) => (
                          <HiveSelect.Option key={platform.id + modelName} value={platform.id + '|' + modelName}>
                            {modelName}
                          </HiveSelect.Option>
                        ))}
                      </HiveSelect.OptGroup>
                    ...
                    <Tooltip
                      content={
                        <div>
                          ...
                          <a href='https://github.com/iOfficeAI/HiveMind/wiki/HiveMind-Image-Generation-Tool-Model-Configuration-Guide' target='_blank' rel='noopener noreferrer' className='text-[rgb(var(--primary-6))] hover:text-[rgb(var(--primary-5))] underline ml-4px' onClick={(e) => e.stopPropagation()}>
                            {t('settings.configGuide')}
                          </a>
                        </div>
                      }
                    >
                      <a href='https://github.com/iOfficeAI/HiveMind/wiki/HiveMind-Image-Generation-Tool-Model-Configuration-Guide' target='_blank' rel='noopener noreferrer' className='ml-8px text-[rgb(var(--primary-6))] hover:text-[rgb(var(--primary-5))] cursor-pointer' onClick={(e) => e.stopPropagation()}>
