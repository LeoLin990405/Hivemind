...
import { HIVEMIND_TIMESTAMP_REGEX } from '@/common/constants';

// 清理HiveMind时间戳后缀，返回原始文件名
export function cleanHiveMindTimestamp(fileName: string): string {
  return fileName.replace(HIVEMIND_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(filePath: string): string {
  const fileName = filePath.split(/[\\/]/).pop() || '';
  return cleanHiveMindTimestamp(fileName);
}
